...
namespace ThiTracNghiem
{
    public partial class QuanLyCauHoi : Form
    {
        ...
        void set()
        {
            ...
            cbDSCH.SelectedIndexChanged += (s, e) =>
            {
                using (var qlttn = new QLTTNDataContext())
                {
                    ...
                    {
                        bsDapAn.DataSource = qlttn.CauHoiQL.Where(ch => ch.maCH == int.Parse(cbDSCH.SelectedValue.ToString())).SingleOrDefault().DapAns.ToList();
                    }
                }
            };
        }
        ...
        void loadCBCauHoi()
        {
            using (var qlttn = new QLTTNDataContext())
            {
                if (qlttn.CapDoQL.Count() > 0 && qlttn.CauHoiQL.Count() > 0)
                {
                    bsCauHoi.DataSource = (qlttn.CauHoiQL.ToList());
                    cbDSCH.DataSource = bsCauHoi;
                    cbDSCH.DisplayMember = "NoiDung";
                    cbDSCH.ValueMember = "maCH";

                    cbCapDo.DataSource = qlttn.CapDoQL.ToList();
                    cbCapDo.DisplayMember = "NoiDung";
                    cbCapDo.ValueMember = "maCD";
                    cbCapDo.SelectedValue = qlttn.CapDoQL.FirstOrDefault().maCD;
                }
                else
                {
