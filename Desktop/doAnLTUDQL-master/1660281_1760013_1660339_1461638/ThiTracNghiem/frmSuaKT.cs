...
namespace ThiTracNghiem
{
    ...
    public partial class frmSuaKT : Form
    {
        ...
        public frmSuaKT(frmGiaoVien frmgv, GiaoVien GV, int makt, string maKhoi, BindingSource bsKyThi)
        {
            ...
            KTCCN = QLTTN.KyThiQL.Where(kt => kt.maKT == makt).FirstOrDefault();
            BT = QLTTN.BuoiThiQL.Where(BT => BT.maKT == makt && BT.DeThi.maMH == GV.maMH).FirstOrDefault();
            if (BT != null)
            {
                DSHS = QLTTN.BaiLamQL.Where(bl => bl.maKT == BT.maKT && bl.maDT == BT.maDT).
                               ...
                               ToList();
                DT = QLTTN.KyThiQL.Where(kt => kt.maKT == this.makt).FirstOrDefault()
            ...
            btnSuaKT.Click += (s, e) =>
            {
                ...
                try
                {
                    var KTCCN = QLTTN.KyThiQL.Where(kythi => kythi.maKT == makt).FirstOrDefault();
                    ...
                    if (KTCCN.LoaiKT == "ThiThiet")
                    {
                        if (BT != null)
                        {
                            QLTTN.BaiLamQL.DeleteAllOnSubmit(BT.BaiLams);
                            QLTTN.BuoiThiQL.DeleteOnSubmit(BT);
                            QLTTN.SubmitChanges();
                        }
                        BT = new BuoiThi
                        {
                        ...
                        {
                            // tạo buổi thi mới
                            QLTTN.BuoiThiQL.InsertOnSubmit(BT);

                            // thêm các thí sinh vào buổi thi mới này
                            foreach (var mahs in DSMaHSDuocChon)
                            {
                                QLTTN.BaiLamQL.InsertOnSubmit(new BaiLam
                                {
                                    maKT = KTCCN.maKT,
                                    maDT = dsMaDT[0],
                                    maHS = mahs,
        ...
        private void SetQLKT()
        {
            ...
            {
                var ktCanCapNhat = QLTTN.KyThiQL.Where(kt => kt.maKT == makt).FirstOrDefault();
                ...
                lblTgBatDau.Text = $"Thời gian bắt đầu môn {QLTTN.MonHocQL.Where(mh => mh.maMH == GV.maMH).FirstOrDefault().tenMH}: ";
            }
        ...
        private void LoadQLKTDgvHocSinh()
        {
            ...
            try
            {
                if (BT != null)
                {
                    var dshsRanh = QLTTN.HocSinhQL.Where(hs => hs.maKhoi == maKhoi && hs.BaiLams.Where(bl => bl.BuoiThi.NgayGioThi > thoiGianBatDau + thoiGianThi|| bl.BuoiThi.NgayGioThi + bl.BuoiThi.DeThi.ThoiGianLamBai < thoiGianBatDau).Count() == hs.BaiLams.Count && hs.BaiLams.Any(bl => BT.BaiLams.Contains(bl)) == false)
                    .Select(hs => new QLTTNDataContext.MyHocSinh
                    {
                        maHS = hs.maHS,
                        HoTen = hs.HoTen,
                ...
                {
                    var dshsRanh = QLTTN.HocSinhQL.Where(hs => hs.maKhoi == maKhoi && hs.BaiLams.Where(bl => bl.BuoiThi.NgayGioThi > thoiGianBatDau + thoiGianThi|| bl.BuoiThi.NgayGioThi + bl.BuoiThi.DeThi.ThoiGianLamBai < thoiGianBatDau).Count() == hs.BaiLams.Count)
                    .Select(hs => new QLTTNDataContext.MyHocSinh
                    {
                        maHS = hs.maHS,
                        HoTen = hs.HoTen,
        ...
        private void LoadQLKTDgvDeThi()
        {
            using (var QLTTN = new QLTTNDataContext())
            {
                ...
                try
                {
                    BSLichThi.DataSource = QLTTN.DeThiQL.Where(DT => DT.maMH == GV.maMH && DT.maKhoi == maKhoi)
