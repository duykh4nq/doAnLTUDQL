    ...
    public partial class frmHocSinh : Form
    {
        ...
        private void BtnCapNhat_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    if (hs.NguoiDung.MatKhau == txtMatKhauCu.Text)
                    {
                        //hs.NguoiDung.MatKhau = txtMatKhauMoi.Text;
                        qlttn.HocSinhQL.Where(hs1 => hs1.maHS == hs.maHS).FirstOrDefault().NguoiDung.MatKhau = txtMatKhauMoi.Text;
                    }
                    else
                    {
        ...
        private void RbDapAn_CheckedChanged(object sender, EventArgs e)
        {
            ...
            if (rb.Checked)
            {
                ...
                var chDaLam = qlttn.CT_BaiLamQL.Where(ctbl => ctbl.maKT == ktGanNhat.maKT
                                                && ctbl.maDT == btGanNhat.maDT
                                                && ctbl.maHS == hs.maHS
                                                && ctbl.maCH == da.maCH).FirstOrDefault();
                if (chDaLam != null)
                {
                    qlttn.CT_BaiLamQL.DeleteOnSubmit(chDaLam);
                }
                qlttn.CT_BaiLamQL.InsertOnSubmit(new CT_BaiLam
                {
                    maKT = ktGanNhat.maKT,
                    maDT = btGanNhat.maDT,
                    maHS = hs.maHS,
        ...
        private void NopBai()
        {
            ...
            if (ktGanNhat.LoaiKT == "ThiThu")
            {
                MessageBox.Show(" ---------------- KẾT QUẢ ----------------"
                    ...
                    + $"{Environment.NewLine}" + $" - Tổng số câu đúng: {blHocSinh.CT_BaiLams.Where(ctbl => ctbl.DungSai == true).Count()}"
                    , "Kết quả bài làm", MessageBoxButtons.OK);
                qlttn.CT_BaiLamQL.DeleteAllOnSubmit(blHocSinh.CT_BaiLams);
                bl = qlttn.BaiLamQL.Where(bl => bl.maHS == hs.maHS && bl.maKT == ktGanNhat.maKT && bl.maDT == btGanNhat.maDT).FirstOrDefault();
                qlttn.SubmitChanges();
                qlttn.ExecuteCommand($"update BaiLam set DiemSo=-1 where maKT={bl.maKT} and maDT={bl.maDT} and maHS={bl.maHS}");
                this.Close();
                return;
            }
        ...
        {
            bsKyThi.DataSource = qlttn.KyThiQL.Where(kt => kt.LoaiKT == "ThiThiet" && kt.BuoiThis.Where(bt => bt.BaiLams.Where(bl => bl.maHS == hs.maHS && bl.DiemSo != null).Count() > 0).Count() > 0).Select(kt => new { kt.maKT, kt.TenKT }).ToList();
            if (bsKyThi.Count > 0)
            {
        ...
        private void loadDgvDaThi()
        {
            if (cbKyThis.SelectedValue != null)
            {
                ...
                qlttn.Refresh(RefreshMode.OverwriteCurrentValues, qlttn.BaiLamQL.Where(bl => bl.maHS == hs.maHS && bl.maKT == makt));

                var kythi = qlttn.KyThiQL.Where(kt => kt.maKT == makt).FirstOrDefault();
                ...
                try
                {
                    ...
                    {
                        bt.DeThi.MonHoc.tenMH,
                        DiemSo = qlttn.BaiLamQL.Where(bl => bl.maHS == hs.maHS && bl.maDT == bt.maDT && bl.DiemSo != null).FirstOrDefault().DiemSo,
                        SoCauDung = qlttn.BaiLamQL.Where(bl => bl.maHS == hs.maHS && bl.maDT == bt.maDT && bl.DiemSo != null).FirstOrDefault()
                                    .CT_BaiLams.Count(ctbl => ctbl.DungSai == true)
                    }).ToList();
                }
                catch (Exception)
                {
        ...
        {
            bsKyThiThu.DataSource = qlttn.KyThiQL.Where(kt => kt.LoaiKT == "ThiThu" && kt.BuoiThis.Where(bt => bt.BaiLams.Where(bl => bl.maHS == hs.maHS && bl.DiemSo == -1).Count() > 0).Count() > 0).Select(kt => new { kt.maKT, kt.TenKT }).ToList();
        ...
        private void btnThiThu_Click(object sender, EventArgs e)
        {
            if (dgvTTKyThiThu.SelectedRows.Count > 0)
            {
                ...
                btGanNhat = qlttn.BuoiThiQL.Where(bt => bt.maKT == makt && bt.maDT == madt).FirstOrDefault();
