...
namespace ThiTracNghiem
{
    public partial class frmAdmin : Form
    {
        ...
        private void setQLND()
        {
            ...
            qlndTxtMaND.Validating += (s, e) =>
              {
                  ...
                  {
                      if (qlttn.NguoiDungQL.Where(nd => nd.maND == qlndTxtMaND.Text).Count() > 0)
                      {
            ...
            using (var qlttn = new QLTTNDataContext())
            {
                qlndCbKhoi.DataSource = qlttn.KhoiLopQL.Select(kl => kl.maKhoi).ToList();
                qlndCbKhoi.SelectedItem = qlndCbKhoi.Items[0];
                qlndCbLop.DataSource = qlttn.LopHocQL.Where(lh => lh.maKhoi == qlndCbKhoi.SelectedValue.ToString()).Select(lh => lh.maLop).ToList();
            ...
            qlndBtnThem.Click += (s, e) =>
              {
                  using (var qlttn = new QLTTNDataContext())
                  {
                      ...
                      if (malnd == "HS")
                      {
                          ...
                          qlttn.HocSinhQL.InsertOnSubmit(hocsinh);
                          qlttn.SubmitChanges();
                      }
                      else if (malnd == "GV")
                      {
                          ...
                          qlttn.GiaoVienQL.InsertOnSubmit(giaovien);
                          qlttn.CT_GiangDayQL.InsertAllOnSubmit(lctgd);
                          qlttn.SubmitChanges();
                      }
                      else if (malnd == "AD")
                      {
            ...
            qlndBtnSua.Click += (s, e) =>
             {
                 try
                 {
                     using (var qlttn = new QLTTNDataContext())
                     {
                         ...
                         var nguoiDung = qlttn.NguoiDungQL.Where(nd => nd.maND == qlndTxtMaND.Text).FirstOrDefault();
                         if (nguoiDung == null)
                         {
                         ...
                         else if (nguoiDung.maLND == "GV")
                         {
                             ...
                             qlttn.CT_GiangDayQL.DeleteAllOnSubmit(nguoiDung.GiaoVien.CT_GiangDays);
                             foreach (DataGridViewRow row in qlndDgvCTGiangDay.Rows)
                             {
                                 var cell = row.Cells["Chon"] as DataGridViewCheckBoxCell;
                                 if (cell.Value == cell.TrueValue)
                                 {
                                     qlttn.CT_GiangDayQL.InsertOnSubmit(new CT_GiangDay
                                     {
                                         maGV = nguoiDung.maND,
                                         maKhoi = row.Cells["maKhoi"].Value.ToString(),
                                         maLop = row.Cells["maLop"].Value.ToString()
                  ...
                  if (e.ColumnIndex == 0)
                  {
                      ...
                      using (var qlttn = new QLTTNDataContext())
                      {
                          if (malnd == "HS")
                          {
                              ...
                              qlttn.HocSinhQL.DeleteOnSubmit(qlttn.HocSinhQL.Where(hs => hs.maHS == mahs).FirstOrDefault());
                              qlttn.NguoiDungQL.DeleteOnSubmit(qlttn.NguoiDungQL.Where(nd => nd.maND == mahs).FirstOrDefault());
                          }
                          else if (malnd == "GV")
                          {
                              var magv = row.Cells["maGV"].Value.ToString();
                              qlttn.CT_GiangDayQL.DeleteAllOnSubmit(qlttn.CT_GiangDayQL.Where(ctgd => ctgd.maGV == magv));
                              qlttn.GiaoVienQL.DeleteOnSubmit(qlttn.GiaoVienQL.Where(gv => gv.maGV == magv).FirstOrDefault());
                              qlttn.NguoiDungQL.DeleteOnSubmit(qlttn.NguoiDungQL.Where(nd => nd.maND == magv).FirstOrDefault());
                          }
                          else if (malnd == "AD")
                          {
                              ...
                              qlttn.NguoiDungQL.DeleteOnSubmit(qlttn.NguoiDungQL.Where(nd => nd.maND == mand).FirstOrDefault());
                          }
                          try
                          {
                              qlttn.SubmitChanges();
                              ...
                              using (var stream = File.Open(ofd.FileName, FileMode.Open, FileAccess.Read, FileShare.ReadWrite))
                              {
                                  ...
                                  try
                                  {
                                      using (var qlttn = new QLTTNDataContext())
                                      {
                                          ...
                                          {
                                              if (qlttn.NguoiDungQL.Where(nd => nd.maND == row["maND"].ToString()).Count() > 0)
                                              {
                                                  soDongThatBai++;
                                                  continue;
                                              }
                                          ...
                                          {
                                              if (qlttn.HocSinhQL.Where(nd => nd.maHS == row["maHS"].ToString()).Count() > 0)
                                              {
                                                  soDongThatBai++;
                                                  continue;
                                              }
                                          ...
                                          {
                                              if (qlttn.GiaoVienQL.Where(nd => nd.maGV == row["maGV"].ToString()).Count() > 0)
                                              {
                                                  soDongThatBai++;
                                                  continue;
                                              }
                                          ...
                                          {
                                              if (qlttn.CT_GiangDayQL.Where(nd => nd.maGV == row["maGV"].ToString() &&
        ...
        private void QlndDgvND_SelectionChanged(object sender, EventArgs e)
        {
                  ...
                  qlndDgvND.DataSource == bsGV)
            {
                ...
                using (var qlttn = new QLTTNDataContext())
                {
                    ...
                    var ctdg = qlttn.CT_GiangDayQL.Where(ct => ct.maGV == magv).Select(ct => new { ct.maKhoi, ct.maLop }).ToList();
        ...
        private void loadCbLoaiND()
        {
            using (var qlttn = new QLTTNDataContext())
            {
                if (qlttn.LoaiNguoiDungQL.Count() == 0)
                {
                ...
                bsLoaiND.DataSource = qlttn.LoaiNguoiDungQL.ToList();
        ...
        private void loadDgvCTGiangDay()
        {
            ...
            using (var qlttn = new QLTTNDataContext())
            {
                qlndDgvCTGiangDay.DataSource = qlttn.LopHocQL.Select(lh => new { lh.maKhoi, lh.maLop }).ToList();
        ...
        private void loadDgvNguoiDung()
        {
            ...
            using (var qlttn = new QLTTNDataContext())
            {
                ...
                if (maLND == "HS")
                {
                    ...
                    bsHS.DataSource = qlttn.HocSinhQL.Where(hs => hs.NguoiDung.maLND == maLND).Select(hs => new
                    {
                        hs.maHS,
                        hs.HoTen,
                        hs.NgaySinh,
                ...
                else if (maLND == "GV")
                {
                    ...
                    bsGV.DataSource = qlttn.GiaoVienQL.Where(gv => gv.NguoiDung.maLND == maLND).Select(gv => new
                    {
                        gv.maGV,
                        gv.HoTen,
                        gv.NgaySinh,
                ...
                {
                    bsAD.DataSource = qlttn.NguoiDungQL.Where(nd => nd.maLND == "AD").Select(nd => new { nd.maND }).ToList();
