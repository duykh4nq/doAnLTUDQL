...
namespace ThiTracNghiem
{
    public partial class frmDangKy : Form
    {
        ...
        private void LoadDgvDSLop()
        {
            ...
            {
                qlndDgvDSLop.DataSource = qlttn.LopHocs.Select(lh => new { lh.maKhoi, lh.maLop }).ToList();
        ...
        private void loadCBLop()
        {
            ...
            using (var qlttn = new QLTTNDataContext())
            {
                qlndCbLop.DataSource = qlttn.LopHocs.Where(lh => lh.maKhoi == qlndCbKhoi.SelectedValue.ToString()).Select(lh => new { lh.maLop }).ToList();
        ...
        private void loadCBKhoi()
        {
            using (var qlttn = new QLTTNDataContext())
            {
                qlndCbKhoi.DataSource = qlttn.KhoiLops.Select(kl => new { kl.maKhoi }).ToList();
        ...
        private void loadCBLoaiND()
        {
            using (var qlttn = new QLTTNDataContext())
            {
                cbLoaiND.DataSource = qlttn.LoaiNguoiDungs.Select(lnd => new { lnd.maLND, lnd.TenLND }).ToList();
